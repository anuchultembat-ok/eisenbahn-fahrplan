...
Alle Testfälle sind in vier Kategorien gegliedert.
Jede Kategorie hat eine eigene Farbe für die Tabellenzeilen.

\begin{description}
  \item[\textbf{N} – Normalfall] Typische Eingaben ohne Sonderbedingungen
  \item[\textbf{G} – Grenzfall] Extremwerte (Minimum/Maximum)
  \item[\textbf{V} – Validierungsfehler] Ungültige Eingaben — werden von \texttt{EingabeValidator} abgewiesen
  \item[\textbf{S} – Spezialfall] Besondere Szenarien (Kaskadeneffekte, Mitternacht-Wrap-around)
\end{description}

% ─── Abkürzungslegende ────────────────────────────────────────────────────────
Strategieabkürzungen in den Strafenspalten:
\textbf{EF} = Einfache Fahrt | \textbf{EW} = Einseitiges Warten | \textbf{BW} = Beidseitiges Warten.
...
\begin{longtable}{p{0.6cm} p{5.5cm} p{2.5cm} p{4.5cm}}
\toprule
\textbf{ID} & \textbf{Eingabe / Datei} & \textbf{Strafe EF/EW/BW} & \textbf{Analyse} \\
\midrule
\endhead

\textbf{N1} &
\texttt{01\_beispiel1\_baseline} \newline
Strecke: A – B – C \newline
Abstände: 5, 7 \quad Start: 17 &
0 / 0 / 0 &
Hin [17,30]; Rück [31,44]. Intervalle überlappen sich nicht.
Alle drei Strategien identisch. Kein Kollisionssegment. \\
\midrule

\textbf{N2} &
\texttt{02\_beispiel2\_eineKollision} \newline
Strecke: A – B – C – D – E – F – G \newline
Abstände: 5, 9, 6, 7, 3, 2 \quad Start: 17 &
0 / 256 / 0 &
Segment B→C: Hin [23,32], Rück [17,27] → Überlapp bei Min.\,23–27.
EW: C wartet 16\,min, Strafe $= 16^2 = 256$.
BW: Rückfahrt startet bei 49, keine Kollision. \\
\midrule

\textbf{N3} &
\texttt{03\_beispiel3\_zehnStationen} \newline
Strecke: A – J (10 Stationen) \newline
Abstände: 12,8,14,3,5,21,13,6,8 \quad Start: 17 &
0 / 0 / 0 &
Unterschiedliche Abstände erzeugen keine Überlappung.
Strafe 0 bei allen Strategien. Prüft korrekte Traversierung langer Listen. \\
\bottomrule
\end{longtable}
...
\textbf{G1} &
\texttt{04\_grenz\_zweiStationen} \newline
Strecke: A – B \quad Abstand: 29 \newline
Start: 0 &
0 / 0 / 0 &
Minimale Stationsanzahl (2). Hin endet bei Min.\,29, Rück startet bei 30.
Kein Überlapp (verschiedene Minuten). \texttt{isKollision()} korrekt. \\
\midrule

\textbf{G2} &
\texttt{05\_grenz\_startzeit59\_wraparound} \newline
Strecke: A – B – C \quad Abstände: 5, 7 \newline
Start: 59 &
0 / 0 / 0 &
Maximal gültige Startzeit. Abfahrt A = 59; Ankunft B = $(59+5)\%60 = 4$.
Modulo-60-Wrap-around korrekt. Ausgabe: \texttt{Ab 59 05 13}. \\
\midrule

\textbf{G3} &
\texttt{09\_grenz\_startzeit0} \newline
Strecke: A – B – C – D \quad Abstände: 15, 15, 15 \newline
Start: 0 &
0 / 3136 / 392 &
Hin [0,47]; Rück startet bei 48, Segment A→B: Hin [0,15], Rück [48,3] (Wrap) → Kollision.
EW: 56\,min gesamt, Strafe $56^2 = 3136$.
BW: Aufgeteilt 28/2 = je 14\,min, Strafe $14^2+14^2 = 392$.
Zeigt Vorteil beidseitigen Wartens. \\
\midrule

\textbf{G4} &
\texttt{12\_minimalAbstand1} \newline
Strecke: A – B – C – D – E – F \newline
Abstände: alle 1 \quad Start: 0 &
0 / 0 / 0 &
Minimaler Abstandswert. Hin endet bei Min.\,9, Rück startet bei 10.
Intervalle berühren sich nicht. Kein Fehler bei Abstand = 1. \\
\midrule

\textbf{G5} &
\texttt{13\_stress\_test} \newline
52 Stationen (A–Z, AA–AZ) \newline
Start: 37 &
— &
Prüft zweistellige Stationsnamen (\texttt{String} statt \texttt{char}) und
Laufzeitverhalten bei $n=52$. Programm terminiert korrekt ohne
\texttt{NullPointerException} am Listenende. \\
\midrule

\textbf{G6} &
\texttt{07\_grenz\_erzwungeneKollision} \newline
Strecke: A – B \quad Abstand: 29 \quad Start: 0 &
0 / 0 / 0 &
Grenzfall nahe der Validierungsschranke (Abstand 29 $<$ 30).
Kollisionsfreie Lösung existiert; alle drei Strategien liefern Strafe 0. \\
\bottomrule
\end{longtable}

% ══════════════════════════════════════════════════════════════════════════════
\subsection{Validierungsfehler-Tests}
% ══════════════════════════════════════════════════════════════════════════════

Diese Tests prüfen, ob \texttt{EingabeValidator} fehlerhafte Eingaben korrekt
abweist. Das Programm verarbeitet die Datei \emph{nicht} und gibt stattdessen
die Fehlermeldung auf \texttt{stderr} aus. Keine Ausgabedatei wird erstellt.

\begin{longtable}{p{0.6cm} p{5.5cm} p{5.8cm} p{1.8cm}}
\toprule
\textbf{ID} & \textbf{Eingabe / Datei} & \textbf{Erwartete Fehlermeldung} & \textbf{Status} \\
\midrule
\endhead

\textbf{V1} &
\texttt{val\_01\_startzeit\_ueber59} \newline
Start: 60 &
\texttt{Startzeit 60 ungültig — erlaubter Bereich: 0..59} &
\textcolor{green!60!black}{\checkmark} \\
\midrule

\textbf{V2} &
\texttt{val\_02\_startzeit\_negativ} \newline
Start: -5 &
\texttt{Startzeit -5 ungültig — erlaubter Bereich: 0..59} &
\textcolor{green!60!black}{\checkmark} \\
\midrule

\textbf{V3} &
\texttt{val\_03\_abstand\_null} \newline
Strecke: A – B – C – D \newline
Abstände: 10, 0, 8 &
\texttt{Abstand 1 (0 min) muss mindestens 1 sein} &
\textcolor{green!60!black}{\checkmark} \\
\midrule

\textbf{V4} &
\texttt{val\_04\_abstand\_grenze30} \newline
Strecke: A – B \quad Abstand: 30 &
\texttt{Abstand 0 (30 min) muss kleiner als 30 sein} \newline
\textit{(verhindert BUG-01)} &
\textcolor{green!60!black}{\checkmark} \\
\midrule

\textbf{V5} &
\texttt{val\_05\_eine\_station} \newline
Strecke: A (nur 1 Station) &
\texttt{Mindestens 2 Stationen erforderlich (gefunden: 1)} &
\textcolor{green!60!black}{\checkmark} \\
\midrule

\textbf{V6} &
\texttt{val\_06\_duplikat\_name} \newline
Strecke: A B C B D (B doppelt) &
\texttt{Stationsname 'B' kommt mehrfach vor} &
\textcolor{green!60!black}{\checkmark} \\
\midrule

\textbf{V7} &
\texttt{06\_grenz\_startzeit\_ueber60} \newline
Start: 73 \textit{(früher BUG-02)} &
\texttt{Startzeit 73 ungültig — erlaubter Bereich: 0..59} &
\textcolor{green!60!black}{\checkmark} \\
\midrule

\textbf{V8} &
\texttt{07b\_abstand30\_kollision} \newline
Abstand: 30 \textit{(früher BUG-01)} &
\texttt{Abstand 0 (30 min) muss kleiner als 30 sein} &
\textcolor{green!60!black}{\checkmark} \\
\bottomrule
\end{longtable}

\textbf{Hinweis:} BUG-01 (\texttt{besteStartzeitRueck = -1}) und BUG-02
(nicht normalisierte Startzeit) sind durch die Validierung vollständig
verhindert. Die zugehörigen Testdateien werden abgewiesen, bevor die
Strategien aufgerufen werden.

% ══════════════════════════════════════════════════════════════════════════════
\subsection{Spezialfall-Tests}
% ══════════════════════════════════════════════════════════════════════════════
...
\textbf{S1} &
\texttt{11\_rollover\_mitternacht} \newline
Strecke: A – B – C \quad Abstände: 25, 25 \newline
Start: 55 &
0 / 64 / 0 &
Segment A→B: Hin läuft [55,20] (überschreitet Min.\,59→0).
\texttt{isTimeInInterval} nutzt \texttt{||-Bedingung} korrekt.
EW: 8\,min Wartezeit, Strafe $= 8^2 = 64$. BW: Strafe 0 (andere Startzeit). \\
\midrule

\textbf{S2} &
\texttt{08\_kaskade\_vieleKollisionen} \newline
Strecke: A – B – C – D – E \newline
Abstände: 10, 10, 10, 10 \quad Start: 0 &
0 / 256 / 0 &
Gleichmäßige Abstände. Segment B→C: Überlapp bei Min.\,11–14.
EW: nur C muss warten (Rückwärts-Pass fängt Kaskade ab).
BW: Rück startet bei 44 → keine Kollision. \\
\midrule

\textbf{S3} &
\texttt{10\_symmetrisch\_beidseitig} \newline
Strecke: A – B – C – D – E \newline
Abstände: 20, 5, 5, 20 \quad Start: 0 &
0 / 36 / 0 &
Symmetrische Strecke. Segment B→C: Hin [21,26], Rück [21,27] → Überlapp.
EW: 6\,min, Strafe $= 6^2 = 36$. BW: Startzeit 54 liefert kollisionsfreie Rückfahrt. \\
\midrule

\textbf{S4} &
\texttt{09\_grenz\_startzeit0} (als Spezialfall) \newline
Gleichmäßige Abstände 15 &
0 / 3136 / 392 &
\textbf{Kein Optimum möglich}: Alle 60 Startzeiten führen zu Kollision am
letzten Segment. BW wählt beste verfügbare Startzeit und teilt 28\,min Wartezeit
auf 14+14 auf. Belegt Korrektheit des Fallback-Pfads. \\
\bottomrule
\end{longtable}
...
\begin{lstlisting}[language=bash, caption={run\_tests.sh — automatisiertes Regression-Testing}]
#!/bin/bash
# Baut das Projekt und vergleicht alle Ausgaben gegen Referenzdateien.
# Erster Lauf legt die Referenzdateien an.

JAR_CLASS="de.matse.ihk.Main"
TESTDIR="testfaelle"
OUTDIR="ausgabe"
REFDIR="ausgabe_referenz"
...
for input in "$TESTDIR"/*.txt; do
  ...
  mvn -q exec:java -Dexec.mainClass="$JAR_CLASS" \
      -Dexec.args="$TESTDIR" 2>/dev/null

  if [ ! -f "$actual" ]; then
    echo "[ABGEWIESEN] $name  (Validierungsfehler erwartet)"
    ((PASS++)); continue
  fi
...
% ══════════════════════════════════════════════════════════════════════════════
\subsection{Gesamtübersicht aller Testfälle}
% ══════════════════════════════════════════════════════════════════════════════
...
\toprule
\textbf{ID} & \textbf{Datei (Kurzname)} & \textbf{Kat.} & \textbf{Kernmerkmal} & \textbf{Strafe BW} & \textbf{Status} \\
\midrule
N1 & \texttt{01\_beispiel1\_baseline}       & Normal & Keine Kollision          & 0     & \textcolor{green!60!black}{\checkmark} \\
N2 & \texttt{02\_beispiel2\_eineKollision}   & Normal & Eine Kollision           & 0     & \textcolor{green!60!black}{\checkmark} \\
...
G5 & \texttt{13\_stress\_test}               & Grenz  & 52 Stationen             & —     & \textcolor{green!60!black}{\checkmark} \\
G6 & \texttt{07\_grenz\_erzwungeneKollision} & Grenz  & Abstand = 29 (Max.)      & 0     & \textcolor{green!60!black}{\checkmark} \\
V1 & \texttt{val\_01\_startzeit\_ueber59}    & Valid. & Start = 60               & —     & \textcolor{green!60!black}{\checkmark} \\
V2 & \texttt{val\_02\_startzeit\_negativ}    & Valid. & Start = -5               & —     & \textcolor{green!60!black}{\checkmark} \\
V3 & \texttt{val\_03\_abstand\_null}         & Valid. & Abstand = 0              & —     & \textcolor{green!60!black}{\checkmark} \\
V4 & \texttt{val\_04\_abstand\_grenze30}     & Valid. & Abstand = 30 (BUG-01)    & —     & \textcolor{green!60!black}{\checkmark} \\
V5 & \texttt{val\_05\_eine\_station}         & Valid. & 1 Station                & —     & \textcolor{green!60!black}{\checkmark} \\
V6 & \texttt{val\_06\_duplikat\_name}        & Valid. & Doppelter Name           & —     & \textcolor{green!60!black}{\checkmark} \\
V7 & \texttt{06\_grenz\_startzeit\_ueber60}  & Valid. & Start = 73 (BUG-02)      & —     & \textcolor{green!60!black}{\checkmark} \\
V8 & \texttt{07b\_abstand30\_kollision}      & Valid. & Abstand = 30 (BUG-01)    & —     & \textcolor{green!60!black}{\checkmark} \\
S1 & \texttt{11\_rollover\_mitternacht}      & Spez.  & Mitternacht-Wrap         & 0     & \textcolor{green!60!black}{\checkmark} \\
S2 & \texttt{08\_kaskade\_vieleKollisionen}  & Spez.  & Kaskadierende Kollision  & 0     & \textcolor{green!60!black}{\checkmark} \\
S3 & \texttt{10\_symmetrisch\_beidseitig}    & Spez.  & Symmetrische Strecke     & 0     & \textcolor{green!60!black}{\checkmark} \\
S4 & \texttt{09\_grenz\_startzeit0 (Detail)} & Spez.  & Kein Optimum möglich     & 392   & \textcolor{green!60!black}{\checkmark} \\
\bottomrule
\end{tabular}
\end{center}
