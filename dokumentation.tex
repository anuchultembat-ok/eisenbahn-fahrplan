...
Nachdem \texttt{EingabeLeser} die Rohdaten eingelesen und in ein \texttt{SimulationsDaten}-Objekt
umgewandelt hat, prüft \texttt{EingabeValidator} die semantische Korrektheit
\emph{bevor} eine einzige Strategie aufgerufen wird.
Ungültige Dateien werden übersprungen; alle übrigen Dateien werden normal weiterverarbeitet.

\begin{lstlisting}[caption={Aufruf in Main.java}]
...
if (!fehler.isEmpty()) {
    ...
    continue; // Datei ueberspringen, naechste Datei laden
}
\end{lstlisting}

\subsubsection*{Aufbau des Validators}

\begin{lstlisting}[caption={EingabeValidator -- Einstiegspunkt}]
public static List<String> validiere(SimulationsDaten daten) {
    ...
    return fehler; // leer = gueltig
}
\end{lstlisting}

Die Methode sammelt \emph{alle} Fehler in einer Liste — es wird kein früher Abbruch bei
dem ersten Fehler vorgenommen (außer bei weniger als 2 Stationen, da dort keine
sinnvolle Streckenprüfung möglich ist). So sieht der Benutzer auf einen Blick
alle Probleme einer Datei.

\subsubsection*{Regel V1 – Startzeit im Bereich {[}0, 59{]}}

\begin{lstlisting}[caption={Startzeit-Prüfung}]
private static void validiereStartZeit(int startZeit, List<String> fehler) {
    if (startZeit < 0 || startZeit > 59)
        fehler.add("Startzeit " + startZeit
                 + " ungueltig -- erlaubter Bereich: 0..59");
}
\end{lstlisting}

\textbf{Begründung:} Das gesamte Zeitmodell basiert auf Modulo 60.
Alle Berechnungen nutzen \texttt{(wert + offset) \% 60}.
\begin{itemize}
  \item \textbf{startZeit $> 59$:} Java-Modulo liefert zwar keinen Fehler, aber die Ausgabe
    enthält dann \texttt{Ab 60} oder \texttt{Ab 73} — Werte, die im Modulo-60-Kreis nicht
    definiert sind (BUG-02).
  \item \textbf{startZeit $< 0$:} Java berechnet $(-5) \% 60 = -5$ (negativer Rest), was zu
    negativen Zeitangaben in der Ausgabe führt.
\end{itemize}

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Eingabe} & \textbf{Ergebnis ohne Validator} & \textbf{Ergebnis mit Validator} \\
\midrule
...
\subsubsection*{Regel V2 – Mindestens 2 Stationen}

\begin{lstlisting}[caption={Mindestanzahl Stationen}]
if (plan.getGroesse() < 2) {
    fehler.add("Mindestens 2 Stationen erforderlich (gefunden: "
             + plan.getGroesse() + ")");
    return; // weitere Plan-Pruefungen sinnlos
}
\end{lstlisting}

\textbf{Begründung:} Eine Strecke mit weniger als 2 Stationen hat kein Segment.
Ohne Segment gibt es keine Hin- oder Rückfahrt.
Mit nur einer Station wäre \texttt{head == tail}; alle
\texttt{while (current.getNext() != null)}-Schleifen terminieren sofort
und erzeugen eine leere Ausgabetabelle (stiller Fehler).

Nach diesem Fehler wird \texttt{return} aufgerufen, da die nachfolgenden
Abstandsprüfungen auf einem leeren Plan einen \texttt{NullPointerException} auslösen würden.

\subsubsection*{Regel V3 – Stationsname nicht leer}

\begin{lstlisting}[caption={Leerstring-Prüfung}]
if (name == null || name.isBlank()) {
    fehler.add("Stationsname an Position " + idx + " ist leer");
}
\end{lstlisting}

\textbf{Begründung:} Ein leerer Name kann entstehen, wenn in der Eingabedatei
doppelte Leerzeichen oder Tabulatoren stehen. \texttt{String.split("\\\\s+")} liefert
dann einen Leer-String als Array-Element.
In der Ausgabetabelle würde die leere Station unsichtbar erscheinen;
die Kollisionsprüfung könnte fehlerhaft referenzieren.

\subsubsection*{Regel V4 – Stationsnamen eindeutig}

\begin{lstlisting}[caption={Duplikat-Prüfung mit HashSet}]
...
\textbf{Begründung:} \texttt{HashSet.add()} gibt \texttt{false} zurück, wenn das Element
bereits vorhanden ist — ein eleganter O(1)-Duplikattest.

Zwei Knoten mit gleichem Namen wären in der Ausgabetabelle nicht unterscheidbar.
Außerdem könnte die Kollisionsprüfung semantisch falsche Ergebnisse liefern,
da \texttt{isKollision()} auf den Zeitfeldern der Knoten operiert und nicht auf den Namen.

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Eingabe} & \textbf{Ausgabe ohne Validator} & \textbf{Mit Validator} \\
\midrule
\texttt{A B C B D} & zwei B-Spalten in Tabelle & Fehlermeldung: „B kommt mehrfach vor" \\
\texttt{A B C}     & korrekt & korrekt \\
\bottomrule
\end{tabular}
\end{center}

\subsubsection*{Regel V5 – Abstand mindestens 1 Minute}

\begin{lstlisting}[caption={Mindestabstand-Prüfung}]
if (d <= 0)
    fehler.add("Abstand " + idx + " (" + d
             + " min) muss mindestens 1 sein");
\end{lstlisting}

\textbf{Begründung:} Ein Abstand von 0 bedeutet, dass zwei Stationen
denselben Punkt auf der Strecke belegen.
Die Zeitberechnung degeneriert:
\[
  \text{ankunft}_{B} = \text{abfahrt}_{A} + 0 = \text{abfahrt}_{A}
\]
Hin und Rück haben dann exakt dieselben Zeitstempel auf dem Segment,
was zwar stets als Kollision erkannt wird, aber semantisch unsinnig ist.
Negative Abstände würden die Zeitberechnung rückwärts laufen lassen.

\subsubsection*{Regel V6 – Abstand kleiner als 30 Minuten}

\begin{lstlisting}[caption={Maximalabstand-Prüfung}]
else if (d >= 30)
    fehler.add("Abstand " + idx + " (" + d
             + " min) muss kleiner als 30 sein"
             + " -- bei Abstand >= 30 existiert keine"
             + " kollisionsfreie Loesung");
\end{lstlisting}

\textbf{Mathematische Begründung:}
Für eine 2-Stationen-Strecke mit Abstand $d$ und Startzeit 0 gilt:
\begin{align*}
  \text{Hin belegt:}  &\quad [0,\; d] \\
  \text{Rück belegt:} &\quad [d+1,\; 2d+1]
\end{align*}
Damit kein Überlapp entsteht, muss $2d+1 < 60$ gelten:
\[
  2d + 1 < 60 \;\Longrightarrow\; d \leq 29
\]
Bei $d = 30$ endet die Rückfahrt bei $2 \cdot 30 + 1 = 61 \equiv 1 \pmod{60}$,
d.\,h. das Rückfahrt-Intervall lautet $[31, 1]$ (über die Stundengrenze).
Dieses Intervall überschneidet sich mit dem Hinfahrt-Intervall $[0, 30]$
bei $t = 0$ und $t = 1$ — eine unvermeidliche Kollision.

\textbf{Abgeleitete Schutzwirkung (BUG-01):}
\texttt{BeidseitigesWartenStrategie} testet alle 60 möglichen Rückfahrt-Startzeiten.
Findet sie keine gültige, bleibt \texttt{besteStartzeitRueck = -1}.
Ein anschließender Aufruf \texttt{rueckfahrtMitKollision(plan, -1)} würde
\texttt{Ab -1} in die Ausgabe schreiben. Der Validator verhindert diesen Fall
vollständig, indem $d \geq 30$ abgelehnt wird.

\begin{center}
\begin{tabular}{cccc}
\toprule
\textbf{Abstand} & \textbf{Max. $2d+1$} & \textbf{Kollisionsfrei möglich?} & \textbf{Validator} \\
\midrule
$d = 1$  & 3   & Ja & akzeptiert \\
$d = 29$ & 59  & Ja (knapp) & akzeptiert \\
$d = 30$ & 61 $\equiv$ 1 & Nein & \textbf{abgelehnt} \\
$d = 59$ & 119 $\equiv$ 59 & Nein & \textbf{abgelehnt} \\
\bottomrule
\end{tabular}
\end{center}
...
V2 & Mindest-Stationen  & $\geq 2$ Stationen                & Leere Ausgabe, NullPointer \\
V3 & Name nicht leer    & kein Leer-/Null-String            & Unsichtbare Spalten \\
V4 & Eindeutige Namen   & kein Duplikat                     & Mehrdeutige Tabelle \\
...
\subsection{Überblick und Entwurfsentscheidung}

Die Klasse \texttt{AusgabeManager} ist für die gesamte Formatierung und persistente Speicherung
der Simulationsergebnisse zuständig.
Sie ist bewusst als reine \textbf{Utility-Klasse} entworfen: alle Methoden sind \texttt{static},
es gibt keinen Konstruktor, und die Klasse hält keinen eigenen Zustand.
Diese Entscheidung vermeidet unnötige Instanziierung und hält die Ausgabelogik von der
Berechnungslogik sauber getrennt (\emph{Separation of Concerns}).
...
\midrule
\texttt{druckeEingabe(plan, startZeit)}           & \texttt{public}  & Eingabe-Echo vor den Strategieblöcken \\
\texttt{druckeErgebnis(strategie, plan)}           & \texttt{public}  & Vollständige 7-zeilige Fahrplantabelle \\
\texttt{printRow(label, extraktor, plan, isStation)}& \texttt{private} & Einzelne horizontale Tabellenzeile \\
\texttt{berechneMindestdauer(plan)}                & \texttt{public}  & Fahrzeit ohne Wartezeiten \\
\texttt{speicherErgebnis(dateiname, inhalt)}       & \texttt{public}  & Ergebnis als Textdatei speichern \\
\bottomrule
\end{tabular}
\end{center}

Der Ablauf in \texttt{Main.java} ist fest geregelt: Pro Eingabedatei wird zuerst
\texttt{druckeEingabe()} aufgerufen (einmalig), dann für jede der drei Strategien
\texttt{druckeErgebnis()}. Alle Teilstrings werden in einem \texttt{StringBuilder}
gesammelt und am Ende mit \texttt{speicherErgebnis()} in eine Datei geschrieben.

\subsection{\texttt{druckeEingabe()} — Eingabe-Echo}

Bevor die drei Strategieblöcke ausgegeben werden, druckt \texttt{druckeEingabe()} die
eingelesenen Rohdaten als lesbares Echo.
Dadurch kann der Leser der Ergebnisdatei sofort erkennen, mit welchen Eingaben
die Simulation gerechnet wurde, ohne die Originaldatei öffnen zu müssen.

Die Methode traversiert die doppelt verkettete Liste einmal von \texttt{head} bis \texttt{tail},
um die Stationsnamen mit \texttt{→} verknüpft auszugeben.
Anschließend wird die Liste ein zweites Mal durchlaufen, um die Abstände zwischen je
zwei benachbarten Knoten (\texttt{distanceToNext}) als kommaseparierte Liste zu formatieren.
Die Startzeit wird mit \texttt{\%02d} immer zweistellig dargestellt (z.\,B. \texttt{07} statt \texttt{7}).

\textbf{Beispielausgabe} für Beispiel2:
\begin{verbatim}
Eingabe:
...
Diese Methode erzeugt die vollständige, siebenzeilige Fahrplantabelle für eine Strategie.
Ihr Design folgt dem Prinzip \emph{Don't Repeat Yourself (DRY)}:
Statt sieben separate Schleifen über die Knotenliste zu schreiben, gibt es nur
eine einzige private Hilfsmethode \texttt{printRow()}, die für alle sieben Zeilen
wiederverwendet wird.

\textbf{Aufbau der Tabelle:}
Die Ausgabe ist so strukturiert, dass jeder Bahnhof eine \emph{Spalte} belegt
und jede Zeile ein anderes Zeitattribut zeigt.
Das ist ungewöhnlich — klassische Tabellen haben Datensätze in Zeilen —
aber entspricht dem in der Aufgabenstellung geforderten Format.
...
\midrule
1 & \texttt{An} (Hin)   & Ankunftszeit der Hinfahrt an jedem Bahnhof\\
2 & \texttt{Wa} (Hin)   & Wartezeit der Hinfahrt (leer wenn 0)\\
3 & \texttt{Ab} (Hin)   & Abfahrtszeit der Hinfahrt\\
4 & Bahnhöfe            & Stationsnamen; \texttt{x} nach Kollisionssegmenten\\
5 & \texttt{Ab} (Rück)  & Abfahrtszeit der Rückfahrt\\
6 & \texttt{Wa} (Rück)  & Wartezeit der Rückfahrt (leer wenn 0)\\
7 & \texttt{An} (Rück)  & Ankunftszeit der Rückfahrt\\
\bottomrule
\end{tabular}
\end{center}

Nach der Tabelle folgen drei Statistikzeilen:
\begin{itemize}
  \item \textbf{Gesamtdauer} Hin/Rück: $\text{Mindestdauer} + \text{SummeWartezeitenHin}$ bzw. $+\text{SummeWartezeitenRück}$
  \item \textbf{Summe Wartezeiten} Hin/Rück: Summe aller \texttt{wartezeitHin} bzw. \texttt{wartezeitRueck}-Felder
  \item \textbf{Summe Strafen}: delegiert an \texttt{strategie.getSummeStrafen()}
\end{itemize}

Die Methode \emph{liest} den Plan nur — sie schreibt keine Werte zurück.
Das macht sie sicher für beliebige Aufruffolgen.

\begin{lstlisting}[caption={AusgabeManager.druckeErgebnis() -- vollständig}]
public static String druckeErgebnis(FahrplanStrategie strategie,
                                    BahnhofPlan plan) {
    StringBuilder sb = new StringBuilder();
    sb.append(strategie.getStrategieName()).append(":\n");

    // 7 Tabellenzeilen ueber printRow() aufgebaut
    ...
    sb.append(printRow("   ", n -> String.format("%2s", n.getName()),
        plan, true));                                 // Bahnhofzeile
    ...
    while (cur != null) {
        warteHin   += cur.getWartezeitHin();
        warteRueck += cur.getWartezeitRueck();
        cur = cur.getNext();
    }
    sb.append(String.format(
        "Gesamtdauer Hinfahrt, Rueckfahrt       : %d, %d%n",
        mindestdauer + warteHin, mindestdauer + warteRueck));
    sb.append(String.format(
        "Summe Wartezeiten Hinfahrt, Rueckfahrt : %d, %d%n",
        warteHin, warteRueck));
    sb.append(String.format(
        "Summe Strafen                          : %d%n",
...
\texttt{printRow()} ist das technische Herzstück der Ausgabe.
Sie erzeugt eine einzelne horizontale Zeile der Tabelle, indem sie die
doppelt verkettete Liste einmal von \texttt{head} bis \texttt{tail} traversiert.

\textbf{Das Funktionsinterface als Schlüssel:}
Der Parameter \texttt{wertExtraktor} hat den Typ \texttt{Function<BahnhofNode, String>}.
Das ist ein Java-8-Funktionsinterface — es erwartet ein Lambda der Form
\texttt{node -> String}.
Jeder der sieben Aufrufe übergibt ein anderes Lambda, das entscheidet,
welches Feld des Knotens in dieser Zeile angezeigt wird.

So entstehen alle sieben Zeilen mit \emph{derselben} Schleife und \emph{demselben} Abstandsformatierungscode.
Ohne dieses Muster würde die Schleife \texttt{while (current != null)} siebenmal
identisch im Code stehen — eine klassische Code-Duplikation.

\textbf{Kollisionsmarkierung:}
Nur in der Bahnhof-Zeile (\texttt{isStationRow = true}) wird zwischen zwei Knoten
geprüft, ob das Segment eine Kollision hat (\texttt{current.isKollision()}).
Falls ja, wird \texttt{"x "} statt \texttt{"  "} zwischen den Spaltennamen eingefügt.
In allen anderen Zeilen erscheinen immer zwei Leerzeichen als Trennzeichen.

\textbf{Nullwerte:}
Die Felder \texttt{ankunfthin} und \texttt{ankunftrueck} sind \texttt{Integer}-Objekte
(nicht \texttt{int}), weil der erste bzw. letzte Bahnhof in einer Richtung
keine Ankunftszeit hat. Das Lambda gibt in diesem Fall \texttt{"  "} zurück,
sodass die Tabellenspalte leer bleibt statt \texttt{null} zu drucken.

\begin{lstlisting}[caption={printRow() -- Implementierung}]
private static String printRow(
    ...
    row.append(String.format("%-3s", label)); // 3 Zeichen links-buendig

    BahnhofNode current = plan.getHead();
    while (current != null) {
        row.append(wertExtraktor.apply(current)); // Lambda aufrufen
        if (current.getNext() != null)
            row.append(isStationRow && current.isKollision()
                ? "x " : "  ");  // Kollisionsmarkierung nur in Bahnhofzeile
...
\textbf{Beispiel:} Aufruf für die Ankunftszeile der Hinfahrt:
\begin{lstlisting}
printRow("An ", n -> n.getAnkunfthin() != null
    ? String.format("%02d", n.getAnkunfthin()) : "  ", plan, false);
\end{lstlisting}
Das Lambda liefert für Bahnhof A (erster Bahnhof, hat keine Ankunft)
\texttt{"  "} und für alle anderen Bahnhöfe z.\,B. \texttt{"22"} oder \texttt{"04"} (zweistellig).

\subsection{\texttt{berechneMindestdauer()} — Basisfahrtdauer}

Die Mindestdauer ist die Fahrtzeit, die \emph{unabhängig von der Strategie} immer anfällt.
Sie setzt sich zusammen aus:
\begin{enumerate}
  \item \textbf{Summe aller Streckenabstände} (\texttt{distanceToNext}): die reine Fahrzeit
    auf den Segmenten.
  \item \textbf{Haltezeiten an Zwischenbahnhöfen}: An jedem Bahnhof außer dem ersten und dem
    letzten hält der Zug genau 1 Minute. Bei $n$ Stationen gibt es $n-2$ Zwischenbahnhöfe.
\end{enumerate}

\textbf{Formel:}
\[
  \text{Mindestdauer} = \sum_{i=1}^{n-1} d_i + (n-2)
  \quad \text{für } n \geq 3 \text{ Stationen}
\]
Für $n = 2$ Stationen (kein Zwischenbahnhof) gilt $\text{Mindestdauer} = d_1$.

\textbf{Beispiel} Strecke A–B–C–D–E–F–G mit Abständen 5, 9, 6, 7, 3, 2:
\[
  \text{Mindestdauer} = (5+9+6+7+3+2) + (7-2) = 32 + 5 = 37\text{ min}
\]

Die tatsächliche Gesamtdauer einer Strategie ergibt sich dann als:
\[
  \text{Gesamtdauer} = \text{Mindestdauer} + \sum \text{wartezeitHin}
  \quad \text{bzw.} \quad + \sum \text{wartezeitRueck}
\]

\begin{lstlisting}[caption={berechneMindestdauer()}]
public static int berechneMindestdauer(BahnhofPlan plan) {
    ...
    if (anzahl > 2) dauer += (anzahl - 2); // Haltezeiten Zwischenbahnhoefe
    return dauer;
}
\end{lstlisting}
...
Diese Methode schreibt den vollständig aufgebauten Ausgabe-String in eine Textdatei.
Sie übernimmt dabei zwei Aufgaben:
\begin{enumerate}
  \item \textbf{Verzeichnis anlegen:} Das Unterverzeichnis \texttt{ausgabe/} wird beim
    ersten Aufruf automatisch erstellt, falls es noch nicht existiert.
    Nachfolgende Aufrufe erkennen das Verzeichnis als bereits vorhanden.
  \item \textbf{Dateinamen ableiten:} Aus dem Eingabedateinamen (z.\,B. \texttt{Beispiel2.txt})
    wird der Ausgabedateiname \texttt{Beispiel2\_ergebnis.txt} gebildet, indem
    \texttt{.txt} entfernt und \texttt{\_ergebnis.txt} angehängt wird.
\end{enumerate}

Die Methode verwendet \texttt{try-with-resources} für den \texttt{BufferedWriter},
sodass die Datei auch bei einer Ausnahme garantiert geschlossen wird.
Ein Schreibfehler wird auf \texttt{stderr} ausgegeben, bricht aber die Verarbeitung
weiterer Dateien nicht ab.

\begin{lstlisting}[caption={speicherErgebnis()}]
public static void speicherErgebnis(String dateiname, String inhalt) {
    File verzeichnis = new File("ausgabe");
    if (!verzeichnis.exists()) verzeichnis.mkdir(); // einmalig anlegen

    File zielDatei = new File(verzeichnis,
        dateiname.replace(".txt", "") + "_ergebnis.txt");

    try (BufferedWriter writer =
             new BufferedWriter(new FileWriter(zielDatei))) {
        writer.write(inhalt);
        System.out.println("  Gespeichert: "
            + zielDatei.getAbsolutePath());
    } catch (IOException e) {
...
\subsection{Format der Ausgabe — annotiertes Beispiel}

Die vollständige Ausgabe für Beispiel2, Strategie \textit{Einseitiges Warten}:

\begin{verbatim}
Einseitiges Warten:
An     22  32  39  47  51  54    <- Ankunft Hinfahrt (A hat keine Ankunft)
Wa                               <- Wartezeit Hin (alle 0 -> leer)
Ab 17  23  33  40  48  52  55    <- Abfahrt Hinfahrt
    A   B   C   D   E   F   G    <- Bahnhofnamen (kein 'x' -> keine Kollision hier)
Ab 33  43  33  10  02  58  55    <- Abfahrt Rueckfahrt
Wa         16                    <- Wartezeit Rueck: C wartet 16 min
An 48  42  16  09  01  57        <- Ankunft Rueckfahrt (G hat keine Ankunft)
...
\begin{itemize}
  \item \textbf{Strategiename} (Zeile 1): von \texttt{strategie.getStrategieName()}
  \item \textbf{An / Wa / Ab (oben)}: drei Zeilen der Hinfahrt
  \item \textbf{Bahnhofzeile}: Stationsnamen; bei der \textit{Einfachen Fahrt} erscheint
    z.\,B. \texttt{Bx} wenn das Segment B$\to$C eine Kollision hat
  \item \textbf{Ab / Wa / An (unten)}: drei Zeilen der Rückfahrt
  \item \textbf{Statistik}: Gesamtdauer, Wartezeiten, Strafe —
    alle ausgerichtet für einfaches Lesen im Texteditor
\end{itemize}

\subsection{Vollständiger Quellcode}

\begin{lstlisting}[caption={AusgabeManager.java -- vollständiger Quellcode}]
 ...
 * Alle Methoden sind statisch -- diese Klasse ist nicht zur
 * Instanziierung gedacht.
 */
public class AusgabeManager {

    /**
     * Gibt eine lesbare Zusammenfassung der Eingabedaten aus:
     * Stationsnamen, Streckenabstaende und die geplante Abfahrtszeit.
     * Erscheint einmal pro Datei, vor den drei Strategiebloecken.
     */
    public static String druckeEingabe(BahnhofPlan plan,
                                       int startZeit) {
        StringBuilder sb = new StringBuilder();
        sb.append("Eingabe:\n");
        sb.append("  Strecke   : ");
    ...
    /**
     * Erstellt die vollstaendige 7-zeilige Fahrplantabelle einer
     * Strategie samt Statistikblock.
     */
    public static String druckeErgebnis(FahrplanStrategie strategie,
                                        BahnhofPlan plan) {
        StringBuilder sb = new StringBuilder();
        ...
        sb.append(printRow("An ", n -> n.getAnkunfthin() != null
            ? String.format("%02d", n.getAnkunfthin())   : "  ",
            plan, false));
        sb.append(printRow("Wa ", n -> n.getWartezeitHin() > 0
            ? String.format("%02d", n.getWartezeitHin())  : "  ",
            plan, false));
        sb.append(printRow("Ab ", n -> n.getAbfahrthin() != null
            ? String.format("%02d", n.getAbfahrthin())   : "  ",
            plan, false));
        sb.append(printRow("   ",
            n -> String.format("%2s", n.getName()), plan, true));
        sb.append(printRow("Ab ", n -> n.getAbfahrtrueck() != null
            ? String.format("%02d", n.getAbfahrtrueck()) : "  ",
            plan, false));
        sb.append(printRow("Wa ", n -> n.getWartezeitRueck() > 0
            ? String.format("%02d", n.getWartezeitRueck()): "  ",
            plan, false));
        sb.append(printRow("An ", n -> n.getAnkunftrueck() != null
            ? String.format("%02d", n.getAnkunftrueck()) : "  ",
            plan, false));
     ...
     * wertExtraktor: Lambda, das den Zellwert aus einem Knoten
     * extrahiert. isStationRow: bei true wird 'x' nach Knoten
     * mit Kollision im Folgesegment angehaengt.
     */
    private static String printRow(
            String label,
            Function<BahnhofNode, String> wertExtraktor,
    ...
    /**
     * Gibt die Mindestfahrtdauer zurueck: Summe aller
     * Streckenabstaende plus je eine Halteminute pro
     * Zwischenbahnhof.
     */
    public static int berechneMindestdauer(BahnhofPlan plan) {
        int dauer = 0;
    ...
    /**
     * Schreibt inhalt in ausgabe/<dateiname ohne .txt>_ergebnis.txt.
     * Erstellt das Ausgabeverzeichnis, falls es nicht existiert.
     */
    public static void speicherErgebnis(String dateiname,
                                        String inhalt) {
        File verzeichnis = new File("ausgabe");
